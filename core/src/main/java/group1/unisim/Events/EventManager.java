...
public class EventManager {
    ...
    public EventManager(Main main){
        Table eventDisplay = new Table(main.getSkin());
    ...
    public void checkEvents() {
        for (int time: eventsRun.keySet()) {
            ...
            if (main.getTimer().getTimeRemaining()< time){
                runEvent();
    ...
    public void updateServices() {
        SatisfactionBar satisfactionBar = main.getSatisfactionBar();
        ContentLoader contentLoader = main.getContentLoader();
        ...
        for (BuildingSlot slot : main.getBuildingSlots()) {
        ...
        for (Service service : values()) {
            ...
            main.getServicesText().get(service).setText(String.format("%d (%d)", services.get(service), servicesUnderConstruction.get(service)));
        ...
        for (var service : values()) {
            main.getAchievementsManager().onServiceValueChange(service, services.getOrDefault(service, 0));
        }
        main.getAchievementsManager().onSatisfactionChange(satisfactionBar.getScore());
    }
